...
class InvoiceManipulator:
    ...
    def create_invoice_types_with_import(cls, data):
        ...
        if invoice_types_data.get("status") == False:
            ...
            invoice_types_dataframe.to_csv(f'{data["application_data"]["PREDATAOUTPUT"]}{data["user_id"]}_invoice_types_{datetime_object}.csv')
            output = {"status":True, "failed_type_groups": failed_invoice_types, "success_type_groups":success_invoice_types, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{data["application_data"]["OUTPUTFOLDER"]}{data["user_id"]}_invoice_types_{datetime_object}.csv'}
            return output
    ...
    def send_invoices_integration(cls, invoices_code, user_id):
        ...
        if user_data.get("data"):
            login_data["client_id"] = user_data["data"][0]["client_id"]
            login_data["client_secret_1"] = user_data["data"][0]["client_secret_1"]
            login_data["client_secret_2"] = user_data["data"][0]["client_secret_2"]

        result  = cls.gather_invoices_data_for_integration(invoices_code, user_id)
        if result.get("data"):
            for obj in result["data"]:
                for i in  obj["invoice_lines"]:
                    if i.get("type_group"):
                        del i["type_group"]
                all_invoices.append(obj)
        try:
            ...
            for invoice_code in invoices_code:
                ...
                # #     result_invoice["data"][0]["invoice_url"] = r.json()["url"]
                    ...
                    update_result = update_result["data"]
                    good_result.append(update_result)
                else:
                    bad_result.append(result["data"][0]["invoice_code"])
            datetime_object = str(datetime.datetime.now()).replace(" ", "")
            json_f = open("utilities/conf/app_info.json")
            json_data = json.load(json_f)
            f = open(f'{json_data["application_data"]["PREDATAOUTPUT"]}{user_id}_sent_object_invoice_{datetime_object}.json', 'w', encoding="utf-8")
            final_data = json.dumps({"data":all_invoices, "login_data":login_data}, ensure_ascii = False)
            f.write(final_data)
            f.close()
            return {"status":True, "good_data":good_result,"bad_data":bad_result, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{json_data["application_data"]["OUTPUTFOLDER"]}{user_id}_sent_object_invoice_{datetime_object}.json'}
    ...
    def gather_invoices_data_for_integration(cls,invoices_code, user_id):
        ...
        obj_for_integration = {"issure_data":{"address":{}},"reciver_data":{"address":{}}, "payment":{}, "delivery":{}, "invoice_lines":[], "taxTotals":[]}
        for invoice_code in invoices_code:
            result = DBManipulator.select_query("invoice", {"$and":[{"invoice_code":invoice_code, "user_id":user_id}]})
            if result.get("data"):
                ...
                try:
                    ...
                    for element in data["invoice_types"]["invoice_type"]:
                        ...
                        full_objects.append(obj_for_integration)
                ...
                return {"status":False}
        return {"status":True, "data":full_objects}
    @classmethod
