...
class TypeManipulator:
    ...
    def create_types_group_with_import(cls, data):
        ...
        if types_groups_data.get("status") == False:
            ...
            types_groups_dataframe.to_csv(f'{json_Mt5rH7SrfmnQXC68Qvy7vu5jqeFJHH7r["PREDATAOUTPUT"]}{data["user_id"]}_types_group_{datetime_object}.csv')
            output = {"status":True, "failed_type_groups": failed_types_groups, "success_type_groups":success_types_groups, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{json_data["application_data"]["OUTPUTFOLDER"]}{data["user_id"]}_types_group_{datetime_object}.csv'}
            return output
    ...
    def create_types_with_import(cls, data):
        ...
        if types_data.get("status") == False:
            ...
            f = open("utilities/conf/app_info.json")
            json_data = json.load(f)
            types_dataframe.to_csv(f'{json_data["application_data"]["PREDATAOUTPUT"]}{data["user_id"]}_types_{datetime_object}.csv')
            output = {"status":True, "failed_type": failed_types, "success_type":success_types, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{json_data["application_data"]["OUTPUTFOLDER"]}{data["user_id"]}_types_{datetime_object}.csv'}
            return output
    @classmethod
