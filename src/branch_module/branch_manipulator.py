...
class BranchManipulator:
    ...
    def create_branch_with_import(cls, data):
        ...
        if branchs_data.get("status") == False:
            ...
            branches_dataframe.to_csv(f'{json_data["application_data"]["PREDATAOUTPUT"]}{data["user_id"]}_branches_{datetime_object}.csv')
            if success_branches:
                output = {"status":True, "failed_branches": failed_branches, "success_branches":success_branches, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{json_data["application_data"]["OUTPUTFOLDER"]}{data["user_id"]}_branches_{datetime_object}.csv'}
            else:
