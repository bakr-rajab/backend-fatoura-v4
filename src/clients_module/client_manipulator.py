...
class ClientManipulator:
    ...
    def create_client_with_import(cls, data):
        ...
        if clients_data.get("status") == False:
            ...
            clients_dataframe.to_csv(f'{json_data["application_data"]["PREDATAOUTPUT"]}{data["user_id"]}_clients_{datetime_object}.csv')
            if success_client:
                output = {"status":True, "failed_clients": failed_client, "success_clients":success_client, "file_paht":f'{json_data["application_data"]["APPLICATION_IP"]}:{json_data["application_data"]["APPLICATION_PORT"]}/api/download/{json_data["application_data"]["OUTPUTFOLDER"]}{data["user_id"]}_clients_{datetime_object}.csv'}
            else:
